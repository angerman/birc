...
- Net: `IO.spawn` actor owns `Socket`; UI ↔ actor via `Chan(NetEvt)` / `Chan(NetCmd)` (Data). Live framing is `push_text`; `push` stays the octet framer for laws. The connecting fiber owns the fd (never `Chan(Socket)`). Boot is Data (`Ready`/`Down`/`Timeout`); UI recvs one then `Chan.close` so the loser send cannot wait on a full channel. `--frames N` is one fuel tick per event; `frames=0` live loops are `@unsafe`. Live DNS A is Bend UDP (`dns.bend` encode/send/parse) plus `recv_octets` (Base `UDP.recv` is a String; A rdata `0x80`–`0xC1` would be FFFD). Demo/offline uses an idle actor (Tick/Cont) until `/connect` sends `Dial{host,port}`; C does not tokenize. `Clock.hhmmss` stamps empty `Line.ts` at paint. `--replay FILE` is `File.read` → `feed_all`, fail closed. `--help` is Bend's runtime CLI; birc usage is `--help-irc`.
